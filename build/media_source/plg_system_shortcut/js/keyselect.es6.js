class JoomlaShortcutModal {
  constructor() {
    ...
    for (let x = 0; x < keySelectBtns.length; x += 1) {
      keySelectBtns[x].addEventListener(
        'click',
        this.handleKeySelectClickEvent,
        false,
      );
    }
  }
  ...
  initialiseKeySelectModal() {
    ...
    Object.keys(attrs).forEach((key) => {
      modalDiv.setAttribute(`${key}`, `${attrs[key]}`);
    });

    modalDiv.innerHTML = `<div class="modal-dialog modal-dialog-centered" role="document"><div class="modal-content"><div class="modal-header"><h5 class="modal-title" id="keySelectModalLabel">${Joomla.getOptions(
      'set_shorcut_text',
    )}</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div><div class="modal-body"><div class="p-3"><p>${Joomla.getOptions(
      'current_combination_text',
    )}: <span id="currentKeyCombination"></span></p><div class="form-group"><input type="hidden" id="current_KeyEvent" value="" /><input type="hidden" id="current_keyValue" value="" /><input type="hidden" id="current_hasControl" value="0" /><input type="hidden" id="current_hasShift" value="0" /><input type="hidden" id="current_hasAlt" value="0" /></div><p>${Joomla.getOptions(
      'new_combination_text',
    )}: <span id="newKeyCombination"></span></p></div></div><div class="modal-footer"><button type="button" class="btn btn-primary" data-bs-dismiss="modal" aria-label="Close">${Joomla.getOptions(
      'cancel_button_text',
    )}</button><button type="button" class="btn btn-success" id="saveKeyCombination">${Joomla.getOptions(
      'save_button_text',
    )}</button></div></div></div>`;
    ...
    keySelectModal.addEventListener(
      'keyup',
      this.handleKeyCombinationkeyUpEvent,
      false,
    );
    const saveKeyCombination = document.getElementById('saveKeyCombination');
    saveKeyCombination.addEventListener(
      'click',
      this.handleSaveCombinationkeyUpEvent,
      false,
    );
  }
  ...
  handleSaveCombinationkeyUpEvent(e) {
    ...
    if (document.getElementById('newKeyCombination').textContent) {
      ...
      document.getElementById(
        `jform_params_${currentKeyEventVal}_keyEvent`,
      ).value = document.getElementById('current_keyValue').value;
      document.getElementById(
        `jform_params_${currentKeyEventVal}_hasControl`,
      ).value = document.getElementById('current_hasControl').value;
      document.getElementById(
        `jform_params_${currentKeyEventVal}_hasShift`,
      ).value = document.getElementById('current_hasShift').value;
      document.getElementById(
        `jform_params_${currentKeyEventVal}_hasAlt`,
      ).value = document.getElementById('current_hasAlt').value;
      document.getElementById(
        `jform_params_${currentKeyEventVal}_keySelect_btn`,
      ).textContent = document.getElementById('newKeyCombination').textContent;
      document.getElementById(
        `jform_params_${currentKeyEventVal}_keySelect`,
      ).value = document.getElementById('newKeyCombination').textContent;
    }
